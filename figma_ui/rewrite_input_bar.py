...
# Replace the "Floating Orchestration Panel" to the "Pro model label" entirely.
# We'll use a regex to match from `{/* Left actions */}` up to `{/* Textarea */}`
                             ...
                             <div className="flex flex-col gap-1.5">
                               {ALL_RUNTIME_MODES.filter(m => m.id !== "standard").map((m) => {
                                 ...
                                 return (
                                   <button
                                     key={m.id}
                                     onClick={() => { setSelectedMode(m.id); setIsDropdownOpen(false); }}
...
# Fix remaining instances of isDropdownOpen that might not have been replaced by the first script
