...
async def verify_session_ownership(
    ...
    # Verify cached kernel ownership
    if cache_kernel and hasattr(cache_kernel, 'user_id'):
        if cache_kernel.user_id != user_id:
            logger.critical(
                f"🔴 KERNEL CACHE MISMATCH: Cached kernel {cache_kernel.user_id} "
                ...
                extra={
                    "session_id": session_id,
                    "cached_owner": cache_kernel.user_id
                }
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def validate_kernel_before_use(
    ...
    if kernel._owner_user_id != user_id:
        ...
        raise ValueError("Kernel ownership validation failed")
    ...
    if kernel._session_id != session_id:
        ...
        raise ValueError("Kernel session validation failed")
...
def wrap_kernel_access(func):
    """
    Decorator to validate kernel before execution.
    """
    import functools
    ...
    async def wrapper(session_id: str, kernel: object, user_id: str, *args, **kwargs):
        ...
        try:
            return await func(session_id, kernel, user_id, *args, **kwargs)
        except Exception as e:
            logger.error(
                f"Error during kernel execution: {str(e)}",
                exc_info=True,
