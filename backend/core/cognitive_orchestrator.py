...
class CognitiveOrchestrator:
    ...
    async def _phase1_parallel_execution(
        ...
        When an image is attached, vision-capable models receive the raw image.
        Non-vision models receive a structured text description generated by the
        best available vision model (vision-first strategy).
        """
        system_prompt = STRUCTURED_EXTRACTION_SYSTEM.format(query=query)

        # ── Vision-first: generate image description for non-vision models ──
        ...
        tasks = []
        for model in models:
            model_has_vision = self._model_supports_vision(model["id"])
            if image_b64 and not model_has_vision and vision_description:
                # Non-vision model: augment query with vision description
                augmented_query = (
                    f"[Image Analysis from vision model]\n{vision_description}\n\n"
                    f"[User Query]\n{query}"
                )
                tasks.append(
                    self._call_and_extract(
                        model, augmented_query, system_prompt,
    ...
    def _model_supports_vision(self, model_id: str) -> bool:
        ...
        return bool(spec and getattr(spec, "supports_vision", False))
    ...
    async def _get_vision_description(
        ...
        for vid in vision_priorities:
            if self._model_supports_vision(vid):
                from metacognitive.cognitive_gateway import COGNITIVE_MODEL_REGISTRY
                spec = COGNITIVE_MODEL_REGISTRY.get(vid)
