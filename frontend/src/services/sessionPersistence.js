...
/**
 * ============================================================
 * Sentinel-E — Runtime Safe Session Persistence
 * ============================================================
 *
 * ARCHITECTURE:
 * - authenticated users only
 * - backend canonical chat ownership
 * - no optimistic shadow persistence
 * - no guest fallback hydration
 * - no stale runtime restores
 * ============================================================
 */

const STORAGE_PREFIX = 'sentinel-v3';

const AUTH_USER_KEY =
  `${STORAGE_PREFIX}-auth-user-id`;
...
const hasSessionStorage = () =>
  typeof window !== 'undefined' &&
  canUseStorage(window.sessionStorage);
...
function normalizeUserId(rawUserId) {
  ...
  if (
    ...
    console.warn(
      '[Sentinel-E] Blocked invalid persistence user:',
      value
    );
...
function resolveUserId(userId = null) {
  ...
  const active =
    normalizeUserId(activePersistenceUserId);
  ...
  const stored = normalizeUserId(
    readLocalRaw(AUTH_USER_KEY, null)
  );
...
function normalizeConversation(
  ...
  return {
    ...
    currentResult:
      conversation.currentResult ?? null,

    sessionState:
      conversation.sessionState ?? null,
    ...
    governanceVerdict:
      conversation.governanceVerdict ?? null,

    orchestrationState:
      conversation.orchestrationState ?? null,

    analyticsState:
      conversation.analyticsState ?? null,

    debateState:
      conversation.debateState ?? null,

    tacticalMapState:
      conversation.tacticalMapState ?? null,
...
export function restoreUserSession(
  ...
  const userId = setPersistenceUser(
    options.userId
  );
  ...
  const keys =
    storageKeysForUser(userId);

  const session =
    readLocal(keys.session, null);
...
export function saveConversationHistory(
  ...
  /**
   * Prevent empty optimistic saves
   */
...
export function createNewConversation(
  ...
  /**
   * Backend owns canonical chat ids now.
...
export function reassignConversationId() {
  /**
   ...
   * - duplicate threads
   * - orphan prompts
   * - optimistic ownership conflicts
   */
...
export function clearAllPersistenceState() {
  ...
  try {
    ...
    console.log(
      '[Sentinel-E] Cleared persistence state.'
    );
  } catch (error) {
    console.warn(
      '[Sentinel-E] Failed clearing persistence:',
      error
    );
  }
}
...
export function isSessionHydrating(
  options = {}
) {
  const userId =
    resolveUserId(options.userId);
