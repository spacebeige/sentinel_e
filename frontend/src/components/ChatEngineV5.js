...
/**
 * ============================================================
 * ChatEngine v5 — Canonical Runtime Safe Edition
 * ============================================================
 *
 * FIXES:
 * - Removes duplicate chat creation
 * - Removes optimistic shadow persistence
 * - Backend becomes canonical chat owner
 * - Fixes split question/response threads
 * - Prevents stale guest hydration propagation
 * - Prevents frontend-generated orphan chat IDs
 * - Preserves existing architecture/runtime
 * ============================================================
 */
...
import { getDefaultPipelineSteps }
from '../engines/modeController';

import memoryManager
from '../engines/memoryManager';

import {
  evaluateResponse,
}
from '../engines/cognitiveGovernor';

import {
  checkHealth as apiCheckHealth,
  sendMCOQuery,
  sendDirectModelQuery,
  getChatMessages,
...
export default function ChatEngineV5() {
  ...
  const [subMode, setSubMode] =
    useState(null);

  const [agentMode, setAgentMode] =
    useState(false);

  const [killActive, setKillActive] =
    useState(false);
  ...
  const [lastResponseText, setLastResponseText] =
    useState('');

  const [lastQueryText, setLastQueryText] =
    useState('');

  const [governanceVerdict, setGovernanceVerdict] =
    useState(null);

  const [error, setError] =
    useState(null);

  const [sessionHydrated, setSessionHydrated] =
    useState(false);

  const [input, setInput] =
    useState('');

  const bootstrapUserRef =
    React.useRef(null);
  ...
  const persistedChats =
    useStore((state) => state.chats);

  const historyLoading =
    useStore((state) => state.isLoading);

  const reloadHistory =
    useStore((state) => state.reloadHistory);
  ...
  useEffect(() => {
    ...
    if (conversation) {
      ...
      setGovernanceVerdict(
        conversation.governanceVerdict ||
        null
      );
  ...
  const handleSend =
    async ({ text, file }) => {
      ...
      try {
        ...
        memoryManager.recordMessage(
          assistantMsg,
          mode,
          subMode
        );

        memoryManager.recordAnalytics(
          result
        );
