#!/usr/bin/env python
"""
Cross-Login Persistence Test — Sentinel-E v5.0 (Simplified)
...
async def test_cross_login_persistence():
    ...
    # Create test users with simple identifiers
    user_a_id = f"test_user_a_{uuid4().hex[:8]}"
    user_b_id = f"test_user_b_{uuid4().hex[:8]}"
    
    print(f"\n[SETUP] Creating test chats and messages...")
    ...
    async for db in get_db():
        try:
            ...
            mem1 = await add_user_memory(
                db, user_a_id,
                "preferred_response_length", "concise",
                confidence=90,
                metadata_json={"source": "test"}
            )
            mem2 = await add_user_memory(
                db, user_a_id,
            ...
            print(f"    - {mem1.key}: {mem1.value}")
            print(f"    - {mem2.key}: {mem2.value}")
            
            # Add user preferences for User A
            print(f"\n[STEP 1] User A sets preferences...")
            prefs_a = await upsert_user_preference(
                db,
                user_a_id,
            ...
            print(f"  ✓ Preferences set: {prefs_a.response_style} tone, {prefs_a.tone}")
            ...
            print(f"  ✓ PASS: User A can retrieve all 3 chats after re-login")
            ...
            print(f"  ✓ PASS: User A's memory facts persisted")
            
            prefs_retrieved = await get_user_preference(db, user_a_id)
            if prefs_retrieved:
                print(f"  ✓ Retrieved preferences:")
                print(f"    - Response Style: {prefs_retrieved.response_style}")
                print(f"    - Tone: {prefs_retrieved.tone}")
                print(f"    - Dark Mode: {prefs_retrieved.dark_mode}")
                assert prefs_retrieved.response_style == "concise", "Style not persisted"
                print(f"  ✓ PASS: User A's preferences persisted")
            ...
            print(f"  ✓ PASS: User B sees only their 2 chats")
            ...
            if overlap:
                print(f"  ✗ FAIL: User B can see {len(overlap)} of User A's chats!")
                assert False, "Cross-user data leakage detected"
            else:
                print(f"  ✓ PASS: No data leakage - users are properly isolated")
            ...
            print(f"  ✓ PASS: Messages persist correctly")
